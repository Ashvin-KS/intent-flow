...
// Activity commands
export async function getClipboardHistory(limit: number = 10): Promise<[string, number][]> {
  return await invoke('get_clipboard_history', { limit });
}

export async function getRecentFiles(limit: number = 10): Promise<string[]> {
  return await invoke('get_recent_files', { limit });
}
